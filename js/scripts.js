( function( $ ) {
	...
	*/
    ...
    emailjs.init('Wzg681TOwN8NdhVMr');
	$('#cform').validate({
		rules: {
			name: {
				required: true
		...
		submitHandler: function() {
			let senderName = stripHtml($('#name').val());
			let email = stripHtml($('#email').val());
			let message = stripHtml($('#message').val());
			emailjs.send('service_kre48tp', 'template_js35udm',{
